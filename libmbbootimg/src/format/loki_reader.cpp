                                           ...
                                           uint64_t &gzip_offset_out)
{
    ...
    auto result_cb = [](File &file_, void *userdata, uint64_t offset)
        ...
        auto orig_offset = file_.seek(0, SEEK_CUR);
        if (!orig_offset) {
            return orig_offset.take_error();
        }

        // Seek to flags byte
        auto seek_ret = file_.seek(static_cast<int64_t>(offset + 3), SEEK_SET);
        if (!seek_ret) {
        ...
        seek_ret = file_.seek(static_cast<int64_t>(*orig_offset), SEEK_SET);
        if (!seek_ret) {
            return seek_ret.take_error();
        }
                                            ...
                                            uint32_t &ramdisk_size_out)
{
    ...
    if (!aboot_offset) {
        reader.set_error(ReaderError::FileError,
                         ...
                         to_string(aboot_offset.take_error()).c_str());
        return file.is_fatal() ? RET_FATAL : RET_FAILED;
    }
