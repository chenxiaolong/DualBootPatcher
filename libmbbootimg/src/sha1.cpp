...
#if MB_BYTE_ORDER == MB_BIG_ENDIAN
...
{
    Buf[I & 15] = rol(Buf[(I + 13) & 15] ^ Buf[(I + 8) & 15]
            ^ Buf[(I + 2) & 15] ^ Buf[I & 15], 1);
    return Buf[I & 15];
}
...
void SHA1::pad()
{
    ...
    addUncounted(static_cast<unsigned char>(InternalState.ByteCount >> 29));
            // Shifting to multiply by 8
    addUncounted(static_cast<unsigned char>(InternalState.ByteCount >> 21));
            // as SHA-1 supports bitstreams as well as
    addUncounted(static_cast<unsigned char>(InternalState.ByteCount >> 13));
            // byte.
    addUncounted(static_cast<unsigned char>(InternalState.ByteCount >> 5));
...
std::array<uint8_t, SHA1::HASH_LENGTH> SHA1::hash(span<const unsigned char> Data)
{
    ...
    std::array<uint8_t, HASH_LENGTH> Arr;
