...
Expected<void> SparseFilePrivate::wread(void *buf, size_t size)
{
    MB_PUBLIC(SparseFile);

    auto bytes_read = file_read_fully(*file, buf, size);
    if (!bytes_read) {
        return join_errors(
                make_error<SparseFileError>(SparseFileErrorType::ReadError),
                bytes_read.take_error());
    }

    if (*bytes_read != size) {
        pub->set_fatal(true);
        return make_error<SparseFileError>(
                SparseFileErrorType::UnexpectedEndOfFile,
                format("Requested %" MB_PRIzu " bytes, but only read %"
                       MB_PRIzu " bytes", size, *bytes_read));
    }
...
Expected<void> SparseFilePrivate::wseek(int64_t offset)
{
    auto seek_ret = file->seek(offset, SEEK_CUR);
    if (!seek_ret) {
        return join_errors(
                make_error<SparseFileError>(SparseFileErrorType::SeekError),
                seek_ret.take_error());
    }
 ...
 */
Expected<void> SparseFilePrivate::skip_bytes(uint64_t bytes)
{
    MB_PUBLIC(SparseFile);
    ...
    case Seekability::CAN_READ:
        auto discarded = file_read_discard(*file, bytes);
        if (!discarded) {
            return join_errors(
                    make_error<SparseFileError>(SparseFileErrorType::ReadError),
                    discarded.take_error());
    ...
    MB_UNREACHABLE("Invalid seekability: %d", static_cast<int>(seekability));
}
 ...
 *         error.
 */
Expected<void> SparseFilePrivate::process_sparse_header(const void *preread_data,
                                                        size_t preread_size)
{
    ...
    auto read_ret = wread(reinterpret_cast<char *>(&shdr) + preread_size,
                          sizeof(shdr) - preread_size);
    if (!read_ret) {
        return read_ret.take_error();
    }
    ...
    if (shdr.magic != SPARSE_HEADER_MAGIC) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidSparseMagic,
                format("Expected %08x, but got %08x",
    ...
    if (shdr.major_version != SPARSE_HEADER_MAJOR_VER) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidSparseMajorVersion,
    ...
    if (shdr.file_hdr_sz < sizeof(SparseHeader)) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidSparseHeaderSize,
                format("Expected at least %" MB_PRIzu ", but have %" PRIu32,
                       sizeof(shdr), shdr.file_hdr_sz));
    }

    // Check chunk header size field
    if (shdr.chunk_hdr_sz < sizeof(ChunkHeader)) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidChunkHeaderSize,
                format("Expected at least %" MB_PRIzu ", but have %" PRIu32,
                       sizeof(SparseHeader), shdr.chunk_hdr_sz));
    }

    // Skip any extra bytes in the file header
    auto skip_ret = skip_bytes(shdr.file_hdr_sz - sizeof(SparseHeader));
    if (!skip_ret) {
        return skip_ret.take_error();
    }
 ...
 *
 * \return ChunkInfo if the chunk header is valid. Otherwise, the error.
 */
Expected<ChunkInfo>
SparseFilePrivate::process_raw_chunk(const ChunkHeader &chdr,
                                     uint64_t tgt_offset)
{
    ...
    if (data_size != chunk_size) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidRawChunk,
                format("Number of data blocks (%" PRIu32 ") does not match"
    ...
    ci.src_begin = cur_src_offset - shdr.chunk_hdr_sz;
    ci.src_end = cur_src_offset + data_size;
    ci.raw_begin = cur_src_offset;
    ci.raw_end = cur_src_offset + data_size;
 ...
 */
Expected<ChunkInfo>
SparseFilePrivate::process_fill_chunk(const ChunkHeader &chdr,
                                      uint64_t tgt_offset)
{
    uint32_t data_size = chdr.total_sz - shdr.chunk_hdr_sz;
    uint64_t chunk_size = static_cast<uint64_t>(chdr.chunk_sz) * shdr.blk_sz;
    ...
    if (data_size != sizeof(fill_val)) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidFillChunk,
                format("Data size (%" PRIu32 ") does not match size of 32-bit "
                       "integer", data_size));
    }

    uint64_t src_begin = cur_src_offset - shdr.chunk_hdr_sz;

    auto read_ret = wread(&fill_val, sizeof(fill_val));
    if (!read_ret) {
        return read_ret.take_error();
    }
 ...
 */
Expected<ChunkInfo>
SparseFilePrivate::process_skip_chunk(const ChunkHeader &chdr,
                                      uint64_t tgt_offset)
{
    uint32_t data_size = chdr.total_sz - shdr.chunk_hdr_sz;
    uint64_t chunk_size = static_cast<uint64_t>(chdr.chunk_sz) * shdr.blk_sz;
    ...
    if (data_size != 0) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidSkipChunk,
                format("Data size (%" PRIu32 ") is not 0", data_size));
    }
 ...
 */
Expected<ChunkInfo>
SparseFilePrivate::process_crc32_chunk(const ChunkHeader &chdr,
                                       uint64_t tgt_offset)
{
    uint32_t data_size = chdr.total_sz - shdr.chunk_hdr_sz;
    uint64_t chunk_size = static_cast<uint64_t>(chdr.chunk_sz) * shdr.blk_sz;
    ...
    if (chunk_size != 0) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidCrc32Chunk,
                format("Chunk data size (%" PRIu64 ") is not 0", chunk_size));
    }
    ...
    auto read_ret = wread(&crc32, sizeof(crc32));
    if (!read_ret) {
        return read_ret.take_error();
    }
    ...
    ci.begin = tgt_offset;
    ci.end = tgt_offset;
    ci.src_begin = tgt_offset - shdr.chunk_hdr_sz;
 ...
 * \param tgt_offset Offset of the output file
 *
 * \return Nothing if the chunk header is valid. Otherwise, the error.
 */
Expected<ChunkInfo>
SparseFilePrivate::process_chunk(const ChunkHeader &chdr, uint64_t tgt_offset)
{
    if (chdr.total_sz < shdr.chunk_hdr_sz) {
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidChunkSize,
                format("Total chunk size (%" PRIu32 ") smaller than chunk "
    ...
    default:
        return make_error<SparseFileError>(
                SparseFileErrorType::InvalidChunkType,
                format("%u", chdr.chunk_type));
    }
}
 ...
 */
Expected<void> SparseFilePrivate::move_to_chunk(uint64_t offset)
{
    MB_PUBLIC(SparseFile);
    ...
    while (chunks.size() < shdr.total_chunks) {
        ...
        if (src_begin < cur_src_offset) {
            pub->set_fatal(true);
            return make_error<SparseFileError>(
                    SparseFileErrorType::InternalError,
                    format("src_begin (%" PRIu64 ")"
                           " < cur_src_offset (%" PRIu64 ")",
                           src_begin, cur_src_offset));
        }

        auto skip_ret = skip_bytes(src_begin - cur_src_offset);
        if (!skip_ret) {
            DEBUG("Failed to skip to chunk #%" MB_PRIzu, chunk_num);
            pub->set_fatal(true);
            return skip_ret.take_error();
        }
        ...
        if (!read_ret) {
            DEBUG("Failed to read chunk header for chunk %" MB_PRIzu,
                  chunk_num);
            pub->set_fatal(true);
            return read_ret.take_error();
        }
        ...
        skip_ret = skip_bytes(shdr.chunk_hdr_sz - sizeof(chdr));
        if (!skip_ret) {
            DEBUG("Failed to skip extra bytes in chunk #%" MB_PRIzu "'s header",
                  chunk_num);
            pub->set_fatal(true);
            return skip_ret.take_error();
        }
        ...
        if (chunk_info->end > file_size) {
            pub->set_fatal(true);
            return make_error<SparseFileError>(
                    SparseFileErrorType::InvalidChunkBounds,
                    format("Chunk #%" MB_PRIzu " ends (%" PRIu64 ") after the "
        ...
        if (chunks.size() == shdr.total_chunks
            ...
            return make_error<SparseFileError>(
                    SparseFileErrorType::InvalidChunkBounds,
                    format("Last chunk does not end (%" PRIu64 ") at position"
 ...
 */
Expected<void> SparseFile::open(File *file)
{
 ...
 * // Forward skipping unsupported if FileError is returned with type
 ...
 * // Random seeking unsupported if FileError is returned with type
 * // FileErrorType::UnsupportedSeek
 * \endcode
 *
...
Expected<void> SparseFile::on_open()
{
    ...
    auto n = priv->file->read(&first_byte, 1);
    if (!n) {
        return n.take_error();
    } else if (*n != 1) {
        return make_error<SparseFileError>(
                SparseFileErrorType::UnexpectedEndOfFile, "File is empty");
    }
    priv->cur_src_offset += *n;

    seek_ret = priv->file->seek(-static_cast<int64_t>(*n), SEEK_CUR);
    if (seek_ret) {
    ...
    return priv->process_sparse_header(&first_byte, *n);
}
 ...
 */
Expected<void> SparseFile::on_close()
{
    MB_PRIVATE(SparseFile);
 ...
 * return false and any further attempts to read or seek the sparse file results
 * in undefined behavior as the internal state will be invalid.
 *
 ...
 *         successfully read. Otherwise, the error.
 */
Expected<size_t> SparseFile::on_read(void *buf, size_t size)
{
    MB_PRIVATE(SparseFile);
    ...
    while (size > 0) {
        auto move_ret = priv->move_to_chunk(priv->cur_tgt_offset);
        if (!move_ret) {
            return move_ret.take_error();
        ...
        case CHUNK_TYPE_RAW: {
            ...
            if (raw_src_offset != priv->cur_src_offset) {
                ...
                auto seek_ret = priv->wseek(seek_offset);
                if (!seek_ret) {
                    return seek_ret.take_error();
                }
            }

            auto read_ret = priv->wread(buf, static_cast<size_t>(to_read));
            if (!read_ret) {
                return read_ret.take_error();
            }
        ...
        case CHUNK_TYPE_DONT_CARE:
            ...
            break;
        default:
            MB_UNREACHABLE("Invalid chunk type: %" PRIu16, priv->chunk->type);
        }
 ...
 *         the error.
 */
Expected<uint64_t> SparseFile::on_seek(int64_t offset, int whence)
{
    MB_PRIVATE(SparseFile);
    ...
    auto move_ret = priv->move_to_chunk(new_offset);
    if (!move_ret) {
        return move_ret.take_error();
    }
