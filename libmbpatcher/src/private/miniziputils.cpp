...
bool MinizipUtils::extract_file(unzFile uf, const std::string &directory)
{
    ...
    auto mkdir_ret = io::create_directories(parent_path);
    if (!mkdir_ret) {
        LOGW("%s: Failed to create directory: %s",
             parent_path.c_str(), to_string(mkdir_ret.take_error()).c_str());
    }
    ...
    while ((n = unzReadCurrentFile(uf, buf, sizeof(buf))) > 0) {
        ...
        if (!bytes_written) {
            LOGE("%s: Failed to write file: %s",
                 full_path.c_str(),
                 to_string(bytes_written.take_error()).c_str());
            unzCloseCurrentFile(uf);
            return false;
        }
    }
    ...
    if (!close_ret) {
        LOGE("%s: Failed to close file: %s",
             full_path.c_str(), to_string(close_ret.take_error()).c_str());
        return false;
    }
