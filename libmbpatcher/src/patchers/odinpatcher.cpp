...
bool OdinPatcherPrivate::patch_tar()
{
    ...
    max_bytes = seek_ret.value();

    seek_ret = la_file.seek(static_cast<int64_t>(current_pos.value()),
                            SEEK_SET);
    if (!seek_ret) {
        LOGE("%s: Failed to seek: %s", info->input_path().c_str(),
             seek_ret.error().message().c_str());
...
int OdinPatcherPrivate::la_open_cb(archive *a, void *userdata)
{
    ...
    ret = priv->la_file.open(w_filename.value(), FileOpenMode::ReadOnly);
#else
#  ifdef __ANDROID__
    if (priv->fd >= 0) {
