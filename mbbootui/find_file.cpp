...
std::string Find_File::Find_Internal(const std::string& filename, const std::string& starting_path)
{
    DIR *d;
    std::string new_path, return_path;
    ...
    if (d == nullptr) {
        LOGI("Find_File: Error opening '%s'", starting_path.c_str());
        return "";
    }
