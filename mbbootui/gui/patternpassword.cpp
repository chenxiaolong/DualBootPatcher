...
GUIPatternPassword::GUIPatternPassword(xml_node<>* node) : GUIObject(node)
{
    ...
    mDotImage = mActiveDotImage = nullptr;
    mDotCircle = mActiveDotCircle = nullptr;
    mDotRadius = 50;
    mLineWidth = 35;
    ...
    if (!mDotImage
            || !mDotImage->GetResource()
            || !mActiveDotImage
            || !mActiveDotImage->GetResource()) {
        mDotCircle = gr_render_circle(mDotRadius, mDotColor.red,
                                      mDotColor.green, mDotColor.blue,
                                      mDotColor.alpha);
        mActiveDotCircle = gr_render_circle(mDotRadius / 2,
                                            mActiveDotColor.red,
                                            mActiveDotColor.green,
                                            mActiveDotColor.blue,
                                            mActiveDotColor.alpha);
    } else {
...
void GUIPatternPassword::CalculateDotPositions()
{
...
void GUIPatternPassword::ConnectDot(int dot_idx)
{
    if (mConnectedDotsLen >= mGridSize * mGridSize) {
        LOGE("mConnectedDots in GUIPatternPassword has overflown!");
        return;
    }
...
int GUIPatternPassword::NotifyVarChange(const std::string& varName,
...
{
    std::vector<char> pattern(mConnectedDotsLen);
    for (size_t i = 0; i < mConnectedDotsLen; i++) {
    ...
    if ((mGridSize == 3) || (tw_android_sdk_version >= 23)) {
