...
GUISliderValue::GUISliderValue(xml_node<>* node) : GUIObject(node)
{
    ...
    mHandleImage = nullptr;
    mHandleHoverImage = nullptr;
    mDragging = false;
...
int GUISliderValue::Render()
{
    ...
    if (mShowRange) {
        ...
        gr_textEx_scaleW(mRenderX + mPadding / 2, rangeY, mMinStr.c_str(),
                         fontResource, mRenderW, TOP_LEFT, 0);
        gr_textEx_scaleW(mLineX + mLineW + mPadding / 2, rangeY, mMaxStr.c_str(),
                         fontResource, mRenderW, TOP_LEFT, 0);
    }
    ...
    if (mValueStr && mShowCurr) {
        ...
        gr_textEx_scaleW(mRenderX + (mRenderW / 2 - textW / 2),
                         mSliderY + mSliderH, mValueStr, fontResource,
                         mRenderW, TOP_LEFT, 0);
    }
...
int GUISliderValue::NotifyVarChange(const std::string& varName,
