...
TEST_F(FileWin32Test, ReadSuccessMaxSize)
{
    ...
    ASSERT_EQ(file.read(as_writable_uchars(nullptr, static_cast<size_t>(UINT_MAX) + 1)),
              oc::success(UINT_MAX));
}
#endif
...
TEST_F(FileWin32Test, ReadFailure)
{
    ...
    unsigned char c[1];
    ASSERT_EQ(file.read(c), oc::failure(ec_from_win32(ERROR_INVALID_HANDLE)));
}
...
TEST_F(FileWin32Test, WriteSuccessMaxSize)
{
    ...
    ASSERT_EQ(file.write(as_uchars(nullptr, static_cast<size_t>(UINT_MAX) + 1)),
              oc::success(UINT_MAX));
}
#endif
