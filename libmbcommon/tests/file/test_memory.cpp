...
TEST(FileStaticMemoryTest, ReadInBounds)
{
    ...
    MemoryFile file(as_writable_bytes(span(in, in_size)));
    ASSERT_TRUE(file.is_open());

    ASSERT_EQ(file.read(as_writable_bytes(span(out))), oc::success(1u));
    ASSERT_EQ(out[0], 'x');
}
...
TEST(FileStaticMemoryTest, ReadOutOfBounds)
{
    ...
    ASSERT_EQ(file.read(as_writable_bytes(span(out))), oc::success(0u));
}
...
TEST(FileStaticMemoryTest, WriteInBounds)
{
    ...
    ASSERT_EQ(file.write(as_bytes(span("y", 1))), oc::success(1u));
    ASSERT_EQ(in[0], 'y');
}
...
TEST(FileStaticMemoryTest, WriteOutOfBounds)
{
    ...
    ASSERT_TRUE(file.seek(10, SEEK_SET));
    ASSERT_EQ(file.write(as_bytes(span("y", 1))), oc::success(0u));
}
...
TEST(FileStaticMemoryTest, WriteTooLarge)
{
    ...
    ASSERT_EQ(file.write(as_bytes(span("yz", 2))), oc::success(1u));
    ASSERT_EQ(in[0], 'y');
}
...
TEST(FileDynamicMemoryTest, WriteEmpty)
{
    ...
    ASSERT_EQ(file.write(as_bytes(span("x", 1))), oc::success(1u));
...
TEST(FileDynamicMemoryTest, WriteTooLarge)
{
    ...
    ASSERT_EQ(file.write(as_bytes(span("yz", 2))), oc::success(2u));
