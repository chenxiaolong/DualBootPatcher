...
struct FileBufferedTest : Test
{
    ...
    void open_mem(span<unsigned char> buf)
    {
        ASSERT_TRUE(_mem_file.open(buf));
...
TEST_F(FileBufferedTest, CheckInvalidStates)
{
    ...
    ASSERT_EQ(_buf_file.read({}), error);
    ASSERT_EQ(_buf_file.write({}), error);
...
TEST_F(FileBufferedTest, ReadSuccess)
{
    ...
    {
        unsigned char buf[3] = {};
        ASSERT_EQ(_buf_file.read(buf), oc::success(3u));
    ...
    {
        unsigned char buf[2] = {};
        ASSERT_EQ(_buf_file.read(buf), oc::success(2u));
    ...
    {
        unsigned char buf[1] = {};
        ASSERT_EQ(_buf_file.read(buf), oc::success(1u));
...
TEST_F(FileBufferedTest, ReadFailure)
{
    ...
    EXPECT_CALL(_mock_file, read(_))
            .Times(2)
            .WillOnce(Return(std::make_error_code(std::errc::io_error)))
            .WillOnce(ReturnSpanSizeArg<0>());

    unsigned char buf[2];

    // Fail when filling buffer
    ASSERT_EQ(_buf_file.read(buf), oc::failure(std::errc::io_error));
    ASSERT_THAT(_buf_file.rbuf(), IsEmpty());
...
TEST_F(FileBufferedTest, ReadLineToContainer)
{
    ...
    _buf_file.set_buf_size(2);
    open_mem(span(data, sizeof(data) - 1));
...
TEST_F(FileBufferedTest, ReadLineToSizedSpan)
{
    ...
    char buf[6] = {};
    span s = as_writable_uchars(span(buf, sizeof(buf) - 1));
