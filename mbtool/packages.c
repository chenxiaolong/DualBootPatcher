...
static int parse_tag_package(xmlNode *node, struct packages *pkgs);
static int parse_tag_packages(xmlNode *node, struct packages *pkgs);
...
static void package_cleanup(struct package *pkg)
{
    ...
    free(pkg->primary_cpu_abi);
    free(pkg->secondary_cpu_abi);
    free(pkg->cpu_abi_override);
    ...
    free(pkg->uid_error);
    free(pkg->install_status);
    free(pkg->installer);
}

static int packages_add(struct packages *pkgs, struct package *pkg)
{
    VECTOR_PUSH_BACK(pkgs, pkg, error);
    return 0;
...
static int packages_remove(struct packages *pkgs, struct package *pkg)
{
    VECTOR_POP_VALUE(pkgs, pkg, error);
    return 0;
