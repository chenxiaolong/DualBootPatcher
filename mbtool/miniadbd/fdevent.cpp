...
static void fdevent_init()
{
    ...
    if (epoll_fd < 0) {
        ADB_LOGE(ADB_FDEV, "epoll_create() failed");
        exit(1);
    }
...
static void fdevent_connect(fdevent *fde)
{
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, fde->fd, &ev)) {
        ADB_LOGE(ADB_FDEV, "epoll_ctl() failed\n");
        exit(1);
    }
#endif
}
...
{
    if (fde->fd < 0) {
        FATAL("bogus negative fd (%d)", fde->fd);
    }

    if (fde->fd >= fd_table_max) {
        int oldmax = fd_table_max;
        if (fde->fd > 32000) {
            FATAL("bogus huuuuge fd (%d)", fde->fd);
        }
        if (fd_table_max == 0) {
            fdevent_init();
            fd_table_max = 256;
        ...
        if (fd_table == 0) {
            FATAL("could not expand fd_table to %d entries", fd_table_max);
        }
...
static void fdevent_unregister(fdevent *fde)
{
    if ((fde->fd < 0) || (fde->fd >= fd_table_max)) {
        FATAL("fd out of range (%d)", fde->fd);
    }

    if (fd_table[fde->fd] != fde) {
        FATAL("fd_table out of sync [%d]", fde->fd);
    }
...
void fdevent_destroy(fdevent *fde)
{
    ...
    if (!(fde->state & FDE_CREATED)) {
        FATAL("fde %p not created by fdevent_create()", fde);
    }
    fdevent_remove(fde);
    free(fde);
}
