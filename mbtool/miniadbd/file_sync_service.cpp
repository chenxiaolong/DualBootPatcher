    ...
    for (;;) {
        ...
        *x = 0;
        ...
        if ((ret < 0) && (errno != EEXIST)) {
            ADB_LOGE(ADB_SERV, "mkdir(\"%s\") -> %s", name, strerror(errno));
            *x = '/';
            return ret;
        } else if (ret == 0) {
...
static int fail_message(int s, const char *reason)
{
    ...
    ADB_LOGE(ADB_SERV, "sync: failure: %s", reason);
...
void file_sync_service(int fd, void *cookie)
{
    ...
    for (;;) {
        ADB_LOGD(ADB_SERV, "sync: waiting for command");
        ...
        msg.req.namelen = 0;
        ADB_LOGD(ADB_SERV, "sync: '%s' '%s'", (char*) &msg.req, name);
