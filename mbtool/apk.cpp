...
public:
    ...
    virtual int on_reached_file() override
    {
        ...
        unzFile uf = unzOpen(_curr->fts_accpath);
        if (!uf) {
            LOGE("{}: Failed to open archive", _curr->fts_path);
            return Action::FTS_Skip;
        }
        ...
        if (unzLocateFile(uf, "AndroidManifest.xml", nullptr) != UNZ_OK) {
            LOGE("{}: package does not contain AndroidManifest.xml",
                 _curr->fts_path);
            return Action::FTS_Skip;
        }
        ...
        if (!read_to_memory(uf, &data)) {
            LOGE("{}: Failed to extract AndroidManifest.xml", _curr->fts_path);
            return Action::FTS_Skip;
        }
...
private:
    ...
    bool check_metadata(const std::vector<unsigned char> &data)
    {
        ...
        if (tree.setTo(data.data(), data.size()) != android::NO_ERROR) {
            ...
            if (code == android::ResXMLTree::START_TAG) {
                ...
                if (depth == 1 && name == "manifest") {
                    ...
                    for (int i = 0; i < numattrs; ++i) {
                        ...
                        return string_value == _package.c_str();
                    }

                    LOGE("package attribute not present in <manifest>");
                    return false;
                }
