...
struct FileWriteErrorBuilder {
  ...
  void add_errno_value(int32_t errno_value) { fbb_.AddElement<int32_t>(FileWriteError::VT_ERRNO_VALUE, errno_value, 0); }
  void add_msg(flatbuffers::Offset<flatbuffers::String> msg) { fbb_.AddOffset(FileWriteError::VT_MSG, msg); }
  FileWriteErrorBuilder(flatbuffers::FlatBufferBuilder &_fbb) : fbb_(_fbb) { start_ = fbb_.StartTable(); }
  FileWriteErrorBuilder &operator=(const FileWriteErrorBuilder &);
  flatbuffers::Offset<FileWriteError> Finish() {
    auto o = flatbuffers::Offset<FileWriteError>(fbb_.EndTable(start_, 2));
    return o;
  }
};

inline flatbuffers::Offset<FileWriteError> CreateFileWriteError(flatbuffers::FlatBufferBuilder &_fbb,
...
inline flatbuffers::Offset<FileWriteError> CreateFileWriteErrorDirect(flatbuffers::FlatBufferBuilder &_fbb,
    int32_t errno_value = 0,
    const char *msg = nullptr) {
  return CreateFileWriteError(_fbb, errno_value, msg ? _fbb.CreateString(msg) : 0);
}
...
struct FileWriteResponse FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const FileWriteError *error() const { return GetPointer<const FileWriteError *>(VT_ERROR); }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct FileWriteResponseBuilder {
  ...
  void add_error(flatbuffers::Offset<FileWriteError> error) { fbb_.AddOffset(FileWriteResponse::VT_ERROR, error); }
  ...
  flatbuffers::Offset<FileWriteResponse> Finish() {
    auto o = flatbuffers::Offset<FileWriteResponse>(fbb_.EndTable(start_, 4));
    return o;
  }
};
...
inline flatbuffers::Offset<FileWriteResponse> CreateFileWriteResponseDirect(flatbuffers::FlatBufferBuilder &_fbb,
    ...
    uint64_t bytes_written = 0,
    flatbuffers::Offset<FileWriteError> error = 0) {
  return CreateFileWriteResponse(_fbb, success, error_msg ? _fbb.CreateString(error_msg) : 0, bytes_written, error);
}
