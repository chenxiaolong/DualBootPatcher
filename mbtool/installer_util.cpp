                                     ...
                                     const std::string &output_file)
{
    ...
    // Open input file
    auto open_ret = fin.open(input_file, FileOpenMode::READ_ONLY);
    if (!open_ret) {
        LOGE("%s: Failed to open for reading: %s",
             input_file.c_str(), to_string(open_ret.take_error()).c_str());
        return false;
    }

    // Open output file
    open_ret = fout.open(output_file, FileOpenMode::WRITE_ONLY);
    if (!open_ret) {
        LOGE("%s: Failed to open for writing: %s",
             output_file.c_str(), to_string(open_ret.take_error()).c_str());
        return false;
    }

    // Replace pattern
    auto result_cb = [](File &file, void *userdata, uint64_t offset_)
            -> Expected<FileSearchAction> {
        (void) file;
    ...
    if (!search_ret) {
        LOGE("%s: Error when searching for pattern: %s",
             input_file.c_str(), to_string(search_ret.take_error()).c_str());
        return false;
    }
    ...
    if (!seek_ret) {
        LOGE("%s: Failed to seek to beginning: %s",
             input_file.c_str(), to_string(seek_ret.take_error()).c_str());
        return false;
    }
    ...
    if (offset) {
        ...
        if (!n || *n != sizeof(target_pattern)) {
            LOGE("%s: Failed to write target pattern: %s",
                 output_file.c_str(), to_string(n.take_error()).c_str());
            return false;
        }
    }
    ...
    if (!close_ret) {
        LOGE("%s: Failed to close file: %s",
             output_file.c_str(), to_string(close_ret.take_error()).c_str());
        return false;
    }
...
bool InstallerUtil::copy_file_to_file(File &fin, File &fout, uint64_t to_copy)
{
    ...
    while (to_copy > 0) {
        ...
        if (!n || *n != to_read) {
            LOGE("Failed to read data: %s", to_string(n.take_error()).c_str());
            return false;
        }

        n = file_write_fully(fout, buf, to_read);
        if (!n || *n != to_read) {
            LOGE("Failed to write data: %s", to_string(n.take_error()).c_str());
            return false;
        }
...
bool InstallerUtil::copy_file_to_file_eof(File &fin, File &fout)
{
    ...
    while (true) {
        ...
        if (!n_read) {
            LOGE("Failed to read data: %s",
                 to_string(n_read.take_error()).c_str());
            return false;
        } else if (*n_read == 0) {
            break;
        }

        auto n_written = file_write_fully(fout, buf, *n_read);
        if (!n_written || *n_written != *n_read) {
            LOGE("Failed to write data: %s",
                 to_string(n_written.take_error()).c_str());
            return false;
        }
    }
